...
# tests/platform/nativeHost.test.ms links the native host against a C mock bridge.
...
for f in $files; do
	case "$f" in
	*style/style.test.ms | *platform/void.test.ms | *platform/voidInput.test.ms | *platform/nativeHost.test.ms)
		echo "== js skip $f"
		continue
		;;
	esac
