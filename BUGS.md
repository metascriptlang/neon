...
| **JS drops the positional arguments of `new C(a, b)`** | `class Pt { x: number; y: number; z: number = 3 }` + `new Pt(1, 2)` | ❌ NEW, measured 2026-08-13 (both before and after the ctor arc — pre-existing). C assigns fields positionally for a ctor-less class and prints `x=1 y=2 z=3`; JS prints `x=undefined y=undefined z=3`. The `z = 3` half was fixed by the arc above; the positional half never existed on JS. Silent wrong answer, no diagnostic. C does it in `codegen/c/expressions.ms` (the Ref-type `new` branch walks `innerType.typeNames`); the JS emitter has no equivalent, and after `ctorLower` every class has a zero-arg synthesized ctor for JS to call. **This is the last red cell in the class-construction matrix and the natural next item** |
