...
| **an optional field of FUNCTION type cannot be omitted** (NEW 2026-08-12, hit widening `fallback` to `NeonView`) | `interface Box { a: int32; cb?: (x: int32) => int32; }` then `const b: Box = { a: 1 };` — `/tmp/optfn/p.ms` | ❌ BOTH lanes: `field 'cb' must be initialized: type 'function' has no default value`. The `?` is ignored for function-typed fields — a reference type defaults to null, a bare closure type has no null to default to. Workaround, measured green on both lanes: spell it `cb?: Cb | null`, which then omits to `null` (`/tmp/optfn/q.ms`). `flow.ms` uses that form for `fallback`. Loud, not silent |
...
| **rest param in a constructor is never packed** | `/tmp/rest/r2_ctor.ms`, `/tmp/rest/r3_inherit_rest.ms` | ❌ `too many arguments to function call, expected 3, have 4`. Root is one line: `restParamLower` opens with `if (node.kind !== NodeKind.CallExpr) return node;` and a constructor call is a **`NewExpr`**. With inheritance on top it degenerates into structurally wrong C (`assigning to 'msString' from incompatible type 'int'`, `no member named '_f2'`) instead of an error. Blocks `constructor(a, ...rest)`, which TypeScript supports (verified with `tsc` 5.9.2) |
| **JS never emits class field initializers** | `/tmp/fi/c1_init_noctor.ms`, `c3_init_plus_ctor.ms` | ❌ `class A { n: number = 7 }` → `new A().n` is `undefined` on JS (C prints 7). Hits **every** class, inheritance or not: `emitClassDeclInner` has `NodeKind.PropertyDecl => {}`. C does it in `codegen/c/expressions.ms` `emitInstanceFieldDefaults`, i.e. a BACKEND — Nim fills defaults in SEM (`sem.nim:658` `defaultFieldsForTheUninitialized`, materialized into the construction AST so no backend knows the concept). Same root family as the stringify row above |
| **C never chains an implicit constructor to the parent** | `/tmp/ctor/d1_parent_ctor_child_none.ms` | ❌ `class P { constructor(){this.w=5} }` + `class K extends P { t = "s" }` → C prints `w=0 t=s` (parent ctor never runs), JS prints `w=5 t=undefined` (ES class chains for free, initializers missing). **The two backends are broken in complementary halves.** TS reference measured in node: field inits run after `super()` and before the ctor body; a ctor-less derived class forwards its args. Design agreed 2026-08-11: one canonical ctor per class = `super(args)` → own field inits → user body, lowered ONCE before codegen, deleting the C-only path |
| **`super(args)` fails across modules** | `/tmp/xmod/main.ms` | ❌ `super() expects 0 arguments, got 1` when the base class is imported. `getCtorParamTypes` (`checkExprPass.ms:2570`) reads `classSym.declNode.classBody` — an imported class symbol has no `declNode` — and it never walks the parent chain either, so a ctor-less middle class also breaks. Blocker for the implicit-ctor design above |
