...
| **indirect interface-extends cycle accepted silently** (NEW 2026-08-06 late, B4 audit round 2; pre-existing by code path — the only B4 touch in the parent-merge loop is the Error-parent skip, which requires a FIRED error, and no error fires here; measured under v0.2.33) | `interface A extends B { a: number; }` + `interface B extends A { b: number; }` — /tmp/b4probes/edge3run.ms builds AND runs, prints both fields | ❌ no diagnostic. Only the DIRECT self-cycle (`A extends A`, resolvePass:1290) is detected; a 2-interface cycle sails through and field visibility depends on resolution order (this probe happened to merge both fields). TS errors on the cycle. Fix direction: cycle detection in the interface parent-resolution walk (visited set), resolvePass phase 8 |
