...
Open framework bugs, and compiler rows not yet re-measured onto a card. The workspace keeps
compiler bugs as cards in `~/metascript/.inbox/compiler/` and, once registered, as `L<n>` entries
in `~/metascript/recompiler/docs/KNOWN-ISSUES.md`; §2 below predates that and is being moved there
row by row (`~/metascript/.wt/neon-bugs-split.md`).
...
---
## §2 — Compiler rows not yet moved to a card (213 rows: 112 in the five tables, 101 bullets; 124 carry no closed marker)
...
- **OPEN compiler — registered as recompiler `docs/KNOWN-ISSUES.md` L47 (`6241cce5`), re-measured 2026-09-20 on binary `94c23bfd`, v0.2.55: still red on BOTH lanes, two errors, unchanged; the same `if` body without the closure is green.** Standalone repro (no Neon import) `probe/l011_constNarrowClosure.ms`. Was: **(2026-09-17, PRE-EXISTING, found by Lát 0.11; red on both binaries above) the narrowing of a `const` does not survive into a closure.** `const g = h; if (g !== null) { const k = (): void => { g(2); }; }` → `callee is possibly null (function | null)`; `const t = s; if (t !== null) { const len = (): number => t.length; }` → `Property 'length' does not exist on type 'Maybe_p1'`. Repro `probe/l011_constNarrowClosure.ms`. TypeScript keeps the narrowing for a `const` (and a never-reassigned parameter) captured by a function expression: the flow walk continues from the function's flow container into the enclosing flow. Not fixed. Neon impact: a nullable `onChangeText` cannot be attached conditionally (its text-unwrap wrapper is a closure over the narrowed temp), so `direct` rejects it with "a nullable onChangeText handler is not supported yet" (`tests/macros/optChangeText{Template,Flat}Rejected.ms`); lift that error when this row closes.
...
## §5 — Fixed, still unverified

Fourteen July/August rows that each claimed a fix living in a recompiler worktree, never
measured since. Until one is run against the installed `msc`, none of them is evidence: the
fix is either in the binary already or the bug is still red. Re-measure a row before quoting
it, then close it here or move it to a card in `~/metascript/.inbox/compiler/`.

The rows whose fix carried a commit SHA were dropped 2026-09-20; the history of this file
holds them.
