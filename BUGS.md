...
| **a LOCAL class is rejected by a misleading error** | `function f() { class L { x: number = 3 }; return new L().x }` | ❌ `Undefined variable 'L'` pointed at the **use** site (`new L()`), nothing at the `class L` declaration — the decl parses fine but is never collected into scope (`ctorLower` + `methodToFunction` only walk top-level `programStmts`). Behaviour is SAFE (loud at first use, never a silent wrong answer, and defaults can't half-run), so this is a message-quality debt, not a correctness row. Right fix = a located "local class not supported" error at the ClassDecl; support itself is deliberately not implemented |
