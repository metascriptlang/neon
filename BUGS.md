...
| **generic component fn passed UNAPPLIED to `createComponent` → the inferred anon props type gets NO lifecycle fns in C** (NEW 2026-08-08, found by the D1 direct-emission interleaving probes) | `probe/directForMin.ms` (hand-written direct shape: `createComponent(For, { each: () => items(), children: (item, i) => … })`), `probe/treeForJsx.ms` (tree JSX `<For>` — element.ms emits the same call, SAME error → the hole predates direct emission and was never covered: flow.test.ms only calls `For<number>({…})` applied, which is GREEN) | ❌ C: `call to undeclared function 'msAnon_1463244Destroy'` (+ Copy/WasMoved/Trace) inside component.ms's TU — the anon props struct is DEFINED in both TUs but its lifecycle fns are emitted NOWHERE in the build. Naming is the tell: every anon type with lifecycle fns is hash-named (`msAnon_cns3g6`, …); the broken one is typeid-decimal-named (`msAnon_1463244`) — the instantiation of `For<T>`'s inline anon-object props param (T inferred from the props literal through `createComponent<P>`'s param position) takes a different naming/registration path that the lifecycle-generation pass never sees. Loud C error, not silent. Blocks JSX `<For>`/`<Index>` in BOTH emissions; `<Show>` unaffected (non-generic). Neon keeps For's D1 differential cell disabled (probe/directInterleave.ms cell D) until this closes |
