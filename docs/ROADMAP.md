...
| 5 | iOS host, then Android host — not compiler-blocked: gate with `when (ios) { … }` around `@compile`/`@passC`/`@passL`/`@link` over one extern surface, the shape `void/src/sokol/gpu.ms` ships. Research notes (2026-09-21, from the Nim original + React Native + ion's generator): `docs/IOS.md`, `docs/ANDROID.md` — boilerplate to ion's generator, component mapping from the references | not started (research done) | large |
