...
| # | work | state | size |
|---|---|---|---|
| 1 | Mobile App Foundation — the same tracked Neon app generated by Ion, compiled/linked by msc, signed and installed on physical iOS and Android devices. Debug/Release, real touch, Yoga layout, rotation/resize, safe area, background/resume, native teardown and signed IPA/APK/AAB; no hand-edited native project or duplicated source inventory. Store upload and product-feature breadth are out. Platform evidence and ownership: `docs/IOS.md`, `docs/ANDROID.md` | active: iOS generated-project/simulator host with safe-area, teardown and rotation proof landed; physical signing open. Android shares the host and runs the counter on the emulator through Ion's Gradle project; the press is parked on the Java handoff (`docs/ANDROID.md` §9) | large |
...
| 5 | RN components: Image, ScrollView, Button, Switch, FlatList, Modal, SafeAreaView — blocked until Mobile App Foundation #1 is green on both platforms | after #1 | medium |
