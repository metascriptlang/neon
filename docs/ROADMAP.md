...
| 5 | `{a && <X/>}` / ternary / `.map` lower to `Show`/`For` | neon macros | done for the three forms, re-measured 2026-09-20: `&&` (`probe/rm5a_and.ms`), `?:` (`rm5b_ternary.ms`) and `{xs.map((v: number): NeonNode => element(<s/>))}` (`rm5h_mapOneParam.ms`) each mount and update. A row arrow that types the index as `number` instead of `Accessor<number>` is rejected — a wrong signature, not a gap (`rm5d`/`rm5f`/`rm5g`); the message names neither side (recompiler L48). `.map(namedFn)` and `||` stay PARKED (BUGS §3). `lowerJsxChild` no longer exists. `Show` / `For` must be in the user's scope: the macro emits bare calls, so a file using only `&&` still needs the import |
